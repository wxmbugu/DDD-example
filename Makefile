...
migrateup:
	 migrate -path db/migrations -database "postgresql://postgres:secret@localhost:5432/patient_tracker?sslmode=disable" -verbose up
migratedown:
	migrate -path db/migrations -database "postgresql://postgres:secret@localhost:5432/patient_tracker?sslmode=disable" -verbose down

.PHONY: postgres startdb migrate migrateup migratedown
